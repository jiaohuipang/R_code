...
setwd(file_path)
df<-unique(read.xlsx('*.xlsx',colNames = T, sheet = 1))
gene_list_file <- '*.xlsx'
...
if(common==1){
  ...
  my_mu <- toupper(c('Missense_Mutation','Nonsense_Mutation','Frame_Shift','In_Frame_Shift','Splice','Amplification','Deletion','5Flank','Fusion','Female','Male','400XD','400D','60XD','60D','PR','SD','PD','KN046_Lenvatinib_8mg','KN046_Lenvatinib_12mg'))
}

my_mu_nosize <- toupper(c('Missense_Mutation','Nonsense_Mutation','Frame_Shift','In_Frame_Shift','Splice','Amplification','Deletion','5Flank','Fusion','Female','Male','400XD','400D','60XD','60D','PR','SD','PD','KN046_Lenvatinib_8mg','KN046_Lenvatinib_12mg'))
...
#########group colors#########################
if(common==0 & fourth_column==1){
  
  if(length(unique(df$X4))==1){
    if(fourth_column==1){
      col_group<-c('cadetblue1', 'brown1', 'deepskyblue', 'darkgoldenrod1', 'yellow', 'springgreen', 'royalblue1', 'slateblue1')
      col_group<-col_group[color_order]
    }
  }else{
    col_group <- c('cadetblue1', 'brown1', 'deepskyblue', 'darkgoldenrod1', 'yellow', 'springgreen', 'royalblue1', 'slateblue1')
  }
...
if(fourth_column == 1){
  df <- df[order(df$X4), ]
  my_levels <- unique(df$X1)
  df$X1 <- factor(df$X1, levels = my_levels)
}
if(common==1){
...
if(fourth_column == 0){
  ...
  two_col <- unique(df[c('X1', 'X4')])
  fourth <- two_col$X4
  fourth_group <- unique(fourth)
  fourth_group_len <- length(fourth_group)
  col_group_list <- list(type = NULL)
  col_group_list$type <- col_group[1:fourth_group_len]
  names(col_group_list$type) <- fourth_group
  
  fourth_string = "bottom_annotation = HeatmapAnnotation(df = data.frame(type = fourth), col = col_group_list,
  annotation_legend_param = list(title = 'Groups',
  title_gp = gpar(fontsize = my_font),
  labels_gp = gpar(fontsize = my_font),
  grid_height = unit(bottom_bar, 'mm'),
  grid_width = unit(bottom_bar, 'mm')),
  annotation_height = unit(bottom_bar,'mm')),"
  
  whole_function_string <- paste0(basic_function_string, fourth_string, last_function_string)
}
